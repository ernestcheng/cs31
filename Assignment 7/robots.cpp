...
bool Robot::getAttacked(int dir)  // return true if dies
{
    ...
    if (!m_arena->determineNewPosition(temp_Row, temp_Col, dir))
        m_health -= 2;
    else
        m_health --;
    
    cerr << "Robot Health:" << m_health << endl;
    
    if (m_health <= 0)
    {
        cerr << "Robot dead at:(" << row() << "," << col() << ")" << endl;
        return true;
    }
    else
        m_arena->determineNewPosition(m_row, m_col, dir);
...
void Player::moveOrAttack(int dir)
{
    ...
    if (m_arena->nRobotsAt(tempRow,tempCol) > 0)
        m_arena->attackRobotAt(tempRow, tempCol, dir);
...
{
    // Find the pointer of a robot which is at r,c.
    
    int k;
    for (k = 0; k < m_nRobots; k++)
        if (m_robots[k]->row() == r && m_robots[k]->col() == c)
            break;
    
    if (m_robots[k]->getAttacked(dir))
    {
        cerr << "Robot " << k << " deleted at: (" << m_robots[k]->row() << "," << m_robots[k]->col() << ")" << endl;
        delete m_robots[k];
        for (int i = k; i < m_nRobots; i++)
